...
use crate::connectors::relay::handler::CommandHandler;
...
impl PayloadSelector {
        ...
        V: Deferred,
    {
        ...
        match inner {
            Inner::Value { input, segment } => Ok(build_value(
                *required,
                input.as_str(),
                segment,
                deferred_key,
            ...
            .map(|payload| CommandHandler::value(node_uri, lane_uri, payload))),
            Inner::Map {
                key_pattern,
                value_pattern,
                remove_when_no_value,
                ...
                match (key, value) {
                    (Some(key_payload), Some(value_payload)) => Ok(Some(CommandHandler::update(
                        node_uri,
                        lane_uri,
                        key_payload,
                        value_payload,
                    ))),
                    (Some(key_payload), None) if *remove_when_no_value => Ok(Some(
                        CommandHandler::remove(node_uri, lane_uri, key_payload),
                    )),
                    _ => Ok(None),
                }
            }
    ...
    V: Deferred,
{
    ...
    match payload {
        ...
        Ok(None) => {
            if required {
                Err(RelayError::Selector(pattern.to_string()))
            } else {
                Ok(None)
            }
        }
