...
use crate::deserialization::{DeserializationError, MessagePart};
use crate::selector::Selector;
use swimos_model::{Attr, Item, Value};

use super::{
    AttrSelector, BasicSelector, ChainSelector, Deferred, IndexSelector, MapLaneSelector,
    MessageField, SelectorComponent, SelectorDescriptor, SlotSelector, ValueLaneSelector,
    ValueSelector,
};
use crate::connectors::generic::config::{MapLaneSpec, ValueLaneSpec};
use crate::connectors::generic::{BadSelector, InvalidLaneSpec};
...
fn value_lane_selector_from_spec_named() {
    ...
    assert_eq!(
        selector.selector,
        ValueSelector::Key(ChainSelector::new(vec![BasicSelector::Index(
            IndexSelector::new(0)
        )]))
    );
}
