...
use swim_api::{agent::Agent, protocol::map::MapMessage};
use swim_model::Text;
use uuid::Uuid;
...
const _: () = {
    ...
    impl MyAgentLifecycle {
        pub fn into_lifecycle(self) -> impl AgentLifecycle<MyAgent> + Clone + Send + 'static {
            let first_lifecycle =
                StatefulValueLaneLifecycle::<MyAgent, MyAgentLifecycle, _>::default()
                    .on_event(MyAgentLifecycle::first_on_event);
            let lanes_lifecycle = ValueLeaf::leaf("first", MyAgent::FIRST, first_lifecycle);
