...
(function(global, factory) {
  ...
  let LeaderboardRowController = (() => {
    ...
    return _a = class LeaderboardRowController extends _classSuper {
      ...
      updateRow(item, rank, change = "none") {
        ...
        const d = item.teamCount.Donut;
        const h = item.teamCount.Hubble;
        ...
        this.teamTeamSpan.attachView().node.innerText = d >= h ? "DON" : "HUB";
        let pctText = `${Math.round((d >= h ? d / (d + h) : h / (d + h)) * 100)}%`;
        if ((d !== null && d !== void 0 ? d : 0) === 0 || (h !== null && h !== void 0 ? h : 0) === 0) {
          pctText = "100%";
        }
  ...
  let LeaderboardTableController = (() => {
    ...
    return _a = class LeaderboardTableController extends _classSuper {
      constructor() {
        super();
        this.LEADERBOARD_SIZE = (__runInitializers(this, _instanceExtraInitializers), 20);
      ...
      _leaderboardDownlink_decorators = [ client.ValueDownlink({
        ...
        didSet(value) {
          ...
          items.forEach(((item, index) => {
            const rank = (index + 1).toFixed(0);
            let rowController = this.owner.getChild(rank, LeaderboardRowController);
            if (!rowController) {
              this.owner.createLeaderboardRowController(rank, item);
              rowController = this.owner.getChild(rank, LeaderboardRowController);
            }
            if (!rowController) {
              return;
            }
            const playerId = item.id;
            const previousPlayerId = rowController.playerId.value;
            let change;
            if (!previousPlayerId || previousPlayerId === playerId) {
              change = "none";
            } else {
              removedPlayers.add(previousPlayerId);
              change = removedPlayers.has(playerId) ? "falling" : "rising";
            }
