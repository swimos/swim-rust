...
use swim::agent::{
    ...
    event_handler::{
        join, Either, EventHandler, HandlerAction, HandlerActionExt, Sequentially,
    },
...
#[derive(Debug, Clone)]
pub struct AgencyAgentLifecycle {
    api: BusesApi,
    agency: Agency,
    poll_delay: Duration,
}

impl AgencyAgentLifecycle {
    pub fn new(api: BusesApi, agency: Agency, poll_delay: Duration) -> Self {
        AgencyAgentLifecycle {
            api,
            agency,
            poll_delay,
        }
    ...
    async fn load_routes(
        ...
        let AgencyAgentLifecycle { api, agency, .. } = self;
...
impl Statistics {
    fn update(mut self, vehicle: &VehicleResponse) -> Self {
        ...
        *min_latitude = min_latitude.min(vehicle.latitude);
        *max_latitude = max_latitude.max(vehicle.latitude);
        *min_longitude = min_longitude.max(vehicle.latitude);
        *max_longitude = max_latitude.max(vehicle.longitude);
        self
    }
}
