...
pub fn ui_server_router(port: u16) -> Router {
    Router::new()
        ...
        .route("/dist/main/swim-traffic.js", get(traffic_js))
        .route("/dist/main/swim-traffic.js.map", get(traffic_js_map))
...
async fn traffic_js() -> impl IntoResponse {
    let headers = [(header::CONTENT_TYPE, JS.clone())];
    (headers, load_file("dist/main/swim-traffic.js").await)
}
...
async fn traffic_js_map() -> impl IntoResponse {
    let headers = [(header::CONTENT_TYPE, JS.clone())];
    (headers, load_file("dist/main/swim-traffic.js.map").await)
}
