...
//! An example demonstrating an MQTT ingress connector.
//!
//! Run the server using the following:
//! ```text
//! $ cargo run --bin mqtt-ingress-connector
//! ```
//!
...
async fn load_config(
    path: Option<String>,
) -> Result<MqttIngressConfiguration, Box<dyn Error + Send + Sync>> {
    let content: String;
    ...
    Ok(MqttIngressConfiguration::from_str(recon)?)
}
