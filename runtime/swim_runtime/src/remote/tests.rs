...
impl RemoteTasksState for FakeRemoteState {
    ...
    fn defer_dns_lookup(&mut self, target: SchemeHostPort, request: PendingRequest) {
        ...
        let PendingRequest::Resolution(resolution_request) = request;
...
async fn transition_request_endpoint_in_table() {
    ...
    let (attach_client_tx, _attach_client_rx) = mpsc::channel(8);
    ...
    state
        .table
        .insert(addr, None, sa, route_tx, attach_client_tx);
    let mut result = Ok(());
...
fn transition_deferred_dns_good_in_table() {
    ...
    state
        .table
        .insert(addr, Some(host.clone()), sa1, route_tx, attach_client_tx);
    let mut result = Ok(());
...
fn transition_deferred_dns_empty() {
    ...
    let err = ConnectionError::Resolution(host.to_string());
