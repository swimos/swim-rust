...
use crate::error::{ConnectionError, Unresolvable};
use crate::remote::table::SchemeHostPort;
use crate::remote::task::AttachClientRouted;
use crate::remote::{AttachClientRequest, RawOutRoute, ResolutionRequest, REQUEST_DROPPED};
...
impl PendingClient {
    ...
    pub async fn send_attach_requests(self) {
        ...
        if let Err(e) = tx.send(routed_request).await {
            ...
            request.send_err_debug(Unresolvable(addr), REQUEST_DROPPED);
        }
    }
}
