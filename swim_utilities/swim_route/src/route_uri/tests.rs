...
fn bad_route_uri_display() {
    let err = InvalidRouteUri("swim://localhost/example".to_string());
    assert_eq!(
        format!("{}", err),
        "'swim://localhost/example' is not a valid route URI."
    )
}
...
fn route_uri_parse_good() {
    ...
    if let Ok(route_uri) = "swim:/example".parse::<RouteUri>() {
        assert_eq!(route_uri.scheme(), Some("swim"));
        assert_eq!(route_uri.path(), "/example");
        assert!(route_uri.query().is_none());
        assert!(route_uri.fragment().is_none());
    } else {
    ...
    if let Ok(route_uri) = "swim:meta:node/unit%2Ffoo/lane/bar".parse::<RouteUri>() {
        assert_eq!(route_uri.scheme(), Some("swim"));
...
fn route_uri_parse_bad() {
    assert!("swim://localhost/example".parse::<RouteUri>().is_err());
...
fn try_from_str_route_uri() {
    ...
    let bad1 = RouteUri::try_from("swim://localhost/example");
...
fn try_from_string_route_uri() {
    ...
    let bad1 = RouteUri::try_from("swim://localhost/example".to_owned());
...
fn segment_iter_encoded() {
    let uri = RouteUri::from_str("swim:meta:node/unit%2Ffoo/lane/bar").unwrap();
...
#[test]
fn segment_iter() {
    let uri = RouteUri::from_str("/swim/meta/node/lane/bar").unwrap();
    let mut iter = uri.path_iter();
...
fn segment_iter_trailing() {
    let uri = RouteUri::from_str("/swim/meta/").unwrap();
    let mut iter = uri.path_iter();
