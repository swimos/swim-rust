...
use crate::model::lifecycle::{for_value_downlink, OnLinked, ValueDownlinkLifecycle};
    ...
    Fut: Future<Output = ()>,
{
    ...
    let (result, _) = join(dl_task, test_body).await;
    result
}
...
/* fn make_lifecycle<T>(tx: mpsc::UnboundedSender<Event<T>>) -> impl for<'a> ValueDownlinkLifecycle<'a, T>
where
    T: Clone,
{
    for_value_downlink()
