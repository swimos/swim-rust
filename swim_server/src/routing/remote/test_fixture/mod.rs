...
pub mod ratchet_fixture {
    use bytes::BytesMut;
    use ratchet::{Extension, Message, NegotiatedExtension, NoExt, Role, WebSocketConfig};
    ...
    #[tokio::test]
    async fn t() {
        let (mut l, mut r) = websocket_pair(NoExt);
        assert!(l.write_text("test").await.is_ok());

        let mut buf = BytesMut::new();
        let ty = r.read(&mut buf).await.unwrap();
