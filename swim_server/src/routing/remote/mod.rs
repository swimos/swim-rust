...
use swim_common::routing::ws::{WebSocketError, WsConnections};
...
use std::io;
use swim_common::routing::server::{ServerConnectionError, Unresolvable};
...
type ResolutionRequest = Request<Result<RoutingAddr, ServerConnectionError>>;
    ...
    match next {
        ...
        Event::Request(RoutingRequest::Endpoint { addr, request }) => {
            let result = if let Some(tx) = state.table_resolve(addr) {
                ...
                Err(Unresolvable(addr.to_string()))
            };
        ...
        Event::Request(RoutingRequest::ResolveUrl { host, request }) => match unpack_url(&host) {
            ...
            _ => {
                request.send_err_debug(
                    ServerConnectionError::Websocket(WebSocketError::Url(host.into_string())),
                    REQUEST_DROPPED,
                );
            }
        },
        ...
        Event::Deferred(DeferredResult::Dns {
            ...
            state.fail_connection(&host, ServerConnectionError::Socket(err.kind()));
        }
        Event::Deferred(DeferredResult::Dns {
            result: Ok(mut addrs),
            host,
            ...
            if let Some(sock_addr) = addrs.next() {
                ...
                state.fail_connection(&host, ServerConnectionError::Resolution);
            }
        }
