...
mod discarding {
    use crate::agent::lane::model::supply::supplier::Supplier;
    use crate::agent::lane::model::supply::{SupplyError, TrySupplyError};
    use futures::future::{ready, BoxFuture};
    use futures::FutureExt;
    use std::marker::PhantomData;

    #[derive(Default)]
    pub(crate) struct DiscardingSupplier<T>(PhantomData<T>);

    impl<T> Supplier<T> for DiscardingSupplier<T>
    where
        T: Send + Sync + 'static,
    {
        fn try_supply(&self, _item: T) -> Result<(), TrySupplyError> {
            Ok(())
        }

        fn supply(&self, _item: T) -> BoxFuture<'_, Result<(), SupplyError>> {
            ready(Ok(())).boxed()
        }
    }
}
