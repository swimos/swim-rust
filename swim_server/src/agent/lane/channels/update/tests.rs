...
fn update_error_display() {
    ...
    let err2 = UpdateError::BadEnvelopeBody(ReadError::UnexpectedItem);
    let string2 = format!("{}", err2);

    assert_eq!(
        string2,
        "The body of an incoming envelope was invalid: Unexpected item in record."
    );
...
fn update_error_from_form_error() {
    let err: UpdateError = ReadError::UnexpectedItem.into();
    assert!(matches!(
        err,
