...
#[tokio::test]
async fn link_to_and_receive_from_supply_lane() {
    let route = RelativePath::new("node", "lane");
    ...
    let uplinks = SupplyLaneUplinks::new(response_rx, route.clone());
    ...
    let assertion_task = async move {
        ...
        assert!(response_tx.send(12).await.is_ok());
        assert!(response_tx.send(17).await.is_ok());
...
#[tokio::test]
async fn link_twice_to_supply_lane() {
    let route = RelativePath::new("node", "lane");
    let (mut response_tx, response_rx) = mpsc::channel(5);
    let (mut action_tx, action_rx) = mpsc::channel(5);
    let (router_tx, mut router_rx) = mpsc::channel(5);
    ...
    let assertion_task = async move {
        ...
        for v in values {
            assert!(response_tx.send(v).await.is_ok());
...
#[tokio::test]
async fn no_messages_after_unlink_from_supply_lane() {
    let route = RelativePath::new("node", "lane");
    let (mut response_tx, response_rx) = mpsc::channel(5);
    let (mut action_tx, action_rx) = mpsc::channel(5);
    let (router_tx, mut router_rx) = mpsc::channel(5);
    ...
    let assertion_task = async move {
        ...
        assert!(response_tx.send(23).await.is_ok());
        assert!(response_tx.send(25).await.is_ok());
