...
/// [`swim_warp::envelope::Envelope`]s to the lanes of an agent and routes
/// [`swim_warp::envelope::Envelope`]s generated by uplinks to those lanes.
pub struct AgentDispatcher<Context> {
    agent_route: RouteUri,
    ...
    Context: AgentExecutionContext + Clone + Send + Sync + 'static,
{
    ...
    /// [`swim_warp::envelope::Envelope`]s and scheduling tasks.
    ...
    ///
    /// # Arguments
    /// * `incoming` - The stream of incoming [`swim_warp::envelope::Envelope`]s to the
    /// agent.
    pub async fn run(
        self,
