...
use crate::router::{ClientConnectionsManager, ClientRoutingRequest, ClientRouterFactory, Router};
...
async fn create_connection_manager() -> (
    Sender<ClientRoutingRequest<Path>>,
    ...
    let (router_request_tx, router_request_rx) = mpsc::channel::<ClientRoutingRequest<Path>>(8);
...
async fn register_connection(
    router_request_tx: &Sender<ClientRoutingRequest<Path>>,
    origin: Origin,
) -> RawRoute {
    ...
    router_request_tx
        .send(ClientRoutingRequest::Connect { request, origin })
        .await
        .unwrap();
...
async fn register_subscriber(
    ...
    router_request_tx
        .send(ClientRoutingRequest::Subscribe {
            target: path,
            request,
        })
        .await
...
async fn test_client_router_lookup() {
    let (request_tx, _request_rx) = mpsc::channel::<ClientRoutingRequest<AbsolutePath>>(8);
...
async fn test_client_router_resolve_sender() {
    let (request_tx, mut request_rx) = mpsc::channel::<ClientRoutingRequest<AbsolutePath>>(8);
    let routing_addr = RoutingAddr::remote(0);
    ...
    tokio::spawn(async move {
        match request_rx.recv().await.unwrap() {
            ClientRoutingRequest::Connect { request, origin } => {
            ...
            ClientRoutingRequest::Subscribe { .. } => {
                unreachable!()
            }
        }
    });
