...
use swim_runtime::routing::{AttachClientRequest, RemoteRoutingRequest};
                ...
                match request {
                    RemoteRoutingRequest::ResolveUrl { host, request } => {
                            ...
                            fake_conns.get_connection(&host)
                        {
                            ...
                            request
                                .send_err(ConnectionError::Resolution(ResolutionError::Host(
                                    host.to_string(),
                                )))
                                .unwrap();
                        }
                    ...
                    RemoteRoutingRequest::AttachClient { request } => {
                        ...
                        if let Some((sender_tx, receiver_rx)) = id_to_conn.get_mut(&addr) {
                            ...
                            request.send(Err(ResolutionError::Addr(addr))).unwrap();
                        }
                    }
                    _ => unreachable!(),
                }
