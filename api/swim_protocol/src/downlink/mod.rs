...
impl From<Vec<u8>> for RawMessage {
    fn from(value: Vec<u8>) -> Self {
        RawMessage(Bytes::from(value))
    }
}

pub struct RawDownlinkOperationEncoder;

impl Encoder<DownlinkOperation<RawMessage>> for RawDownlinkOperationEncoder {
    type Error = std::io::Error;

    fn encode(
        &mut self,
        item: DownlinkOperation<RawMessage>,
        dst: &mut bytes::BytesMut,
    ) -> Result<(), Self::Error> {
        let DownlinkOperation { body } = item;
        let data = body.0.as_ref();
...
pub struct RawDownlinkOperationDecoder;

impl Decoder for RawDownlinkOperationDecoder {
    type Item = DownlinkNotification<RawMessage>;
    type Error = FrameIoError;
    ...
    fn decode(&mut self, src: &mut bytes::BytesMut) -> Result<Option<Self::Item>, Self::Error> {
        ...
        match tag {
            LINKED => {
                src.advance(1);
                Ok(Some(DownlinkNotification::Linked))
            }
            SYNCED => {
                src.advance(1);
                Ok(Some(DownlinkNotification::Synced))
            }
            EVENT => {
                if src.remaining() < TAG_SIZE + LEN_SIZE {
                    let required = TAG_SIZE + LEN_SIZE - src.remaining();
                    ...
                    // todo: Need to peek if there's enough data in the buffer
                    src.advance(1);
                    let len = src.get_u64() as usize;
                    let body = src.split_to(len).freeze();
                    Ok(Some(DownlinkNotification::Event {
            ...
            UNLINKED => {
                src.advance(1);
                Ok(Some(DownlinkNotification::Unlinked))
            }
            t => Err(FrameIoError::BadFrame(InvalidFrame::InvalidHeader {
                problem: Text::from(format!("Invalid downlink notification tag: {}", t)),
            })),
        }
    }
}
