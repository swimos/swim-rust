...
impl<'f> ValidatedField<'f> {
    ...
    pub fn as_attr(&self) -> TokenStream2 {
        ...
        quote! {
            swimos_schema::schema::FieldSpec::default(
                swimos_schema::schema::attr::AttrSchema::named(
                    #ident,
                    #field_schema,
                )
            )
    ...
    pub fn as_item(&self) -> TokenStream2 {
        ...
        let build_named = |name| {
            quote! {
                swimos_schema::schema::ItemSchema::Field(
                    swimos_schema::schema::slot::SlotSchema::new(
                        swimos_schema::schema::StandardSchema::text(#name),
                        #field_schema,
                    )
                )
            }
        ...
        match &form_field.label {
            ...
            Label::Anonymous(_) => quote!(
                swimos_schema::schema::ItemSchema::ValueItem(#field_schema)
            ),
...
/// A representation used derive to `swimos_schema::schema::StandardSchema`. Where fields are
    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        let quote = match self {
            ...
            StandardSchema::Equal(path) => quote!(
                    swimos_schema::schema::StandardSchema::Equal(#path())
            ),
            StandardSchema::OfKind(kind) => {
                quote!(swimos_schema::schema::StandardSchema::OfKind(#kind))
            }
            StandardSchema::IntRange(Range {
                ...
                if *inclusive {
                    quote!(swimos_schema::schema::StandardSchema::inclusive_int_range(#lower, #upper))
                } else {
                    quote!(swimos_schema::schema::StandardSchema::int_range(#lower, #upper))
                }
            }
            StandardSchema::UintRange(Range {
                ...
                if *inclusive {
                    quote!(swimos_schema::schema::StandardSchema::inclusive_uint_range(#lower, #upper))
                } else {
                    quote!(swimos_schema::schema::StandardSchema::uint_range(#lower, #upper))
                }
            }
            StandardSchema::FloatRange(Range {
                ...
                if *inclusive {
                    quote!(swimos_schema::schema::StandardSchema::inclusive_float_range(#lower, #upper))
                } else {
                    quote!(swimos_schema::schema::StandardSchema::float_range(#lower, #upper))
                }
            }
            StandardSchema::BigIntRange(Range {
                ...
                if *inclusive {
                    quote!(swimos_schema::schema::StandardSchema::inclusive_big_int_range(#lower, #upper))
                } else {
                    quote!(swimos_schema::schema::StandardSchema::big_int_range(#lower, #upper))
                }
            }
            StandardSchema::NonNan => quote!(swimos_schema::schema::StandardSchema::NonNan),
            StandardSchema::Finite => quote!(swimos_schema::schema::StandardSchema::Finite),
            StandardSchema::Text(text) => {
                quote!(swimos_schema::schema::StandardSchema::text(&#text))
            }
            StandardSchema::Anything => {
                quote!(swimos_schema::schema::StandardSchema::Anything)
            }
            StandardSchema::Nothing => quote!(swimos_schema::schema::StandardSchema::Nothing),
            StandardSchema::DataLength(len) => {
                quote!(swimos_schema::schema::StandardSchema::binary_length(#len))
            }
            StandardSchema::NumAttrs(num) => {
                quote!(swimos_schema::schema::StandardSchema::NumAttrs(#num))
            }
            StandardSchema::NumItems(num) => {
                quote!(swimos_schema::schema::StandardSchema::NumItems(#num))
            }
            StandardSchema::And(and_schema) => quote!(swimos_schema::schema::StandardSchema::And(
                vec![#(#and_schema,)*]
            )),
            StandardSchema::Or(or_schema) => quote!(swimos_schema::schema::StandardSchema::Or(
                vec![#(#or_schema,)*]
            )),
            StandardSchema::Not(not_schema) => quote!(
                swimos_schema::schema::StandardSchema::Not(std::boxed::Box::new(#not_schema))
            ),
            StandardSchema::AllItems(ts) => {
                quote!(swimos_schema::schema::StandardSchema::AllItems(std::boxed::Box::new(#ts)))
            }
            StandardSchema::None => {
    ...
    fields
        ...
        .map(|form_field| {
            let initial_schema = match &form_field.original.ty {
                Type::Path(path) => {
                    let path = quote! {
                        <#path as swimos_schema::ValueSchema>
                    };
