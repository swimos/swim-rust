...
pub fn build_validated_form(
    ...
    let ts = quote! {
        impl #impl_generics swimos_schema::ValueSchema for #structure_name #ty_generics #where_clause
        {
            fn schema() -> swimos_schema::schema::StandardSchema {
                #type_contents
            }
        }
    };
...
fn type_contents_to_tokens(
    ...
    match type_contents {
        ...
        TypeContents::Enum(EnumRepr { variants, .. }) => {
            ...
            quote! {
                swimos_schema::schema::StandardSchema::Or(vec![
                    #schemas
                ])
            }
        }
    ...
    match &container_schema {
        StandardSchema::AllItems(items) => match compound_type {
            CompoundTypeKind::Labelled => {
                quote! {
                    swimos_schema::schema::StandardSchema::AllItems(
                        std::boxed::Box::new(
                            swimos_schema::schema::ItemSchema::Field(
                                swimos_schema::schema::slot::SlotSchema::new(
                                    swimos_schema::schema::StandardSchema::Anything,
                                    #items,
                                )
                            )
                        )
            ...
            CompoundTypeKind::Tuple | CompoundTypeKind::NewType => {
                quote! {
                    swimos_schema::schema::StandardSchema::AllItems(
                        std::boxed::Box::new(
                            swimos_schema::schema::ItemSchema::ValueItem(#items)
                        )
                    ),
                }
            }
...
fn derive_items(fields: &[ValidatedField], descriptor: &FieldManifest) -> TokenStream2 {
    ...
    if descriptor.replaces_body {
        ...
        return quote!(<#ty as swimos_schema::ValueSchema>::schema());
    }
    ...
    schemas = quote! {
        swimos_schema::schema::StandardSchema::Layout {
            items: vec![
                #schemas
            ],
            exhaustive: true
    ...
    let item_schemas = {
        match &descriptor.schema {
            ...
            _ => {
                ...
                quote! {
                    swimos_schema::schema::StandardSchema::And(vec![
                        #container_schema
                        #item_schema
                    ])
                }
...
fn build_generics(
    ...
    let generics = add_bound(
        ...
        &parse_quote!(swimos_schema::ValueSchema),
    );
