...
pub fn build_head_attribute(
    ...
    let tag_value_schema = match header_body_opt.first() {
            ...
            if !header_schemas.is_empty() {
                quote! {
                    swimos_schema::schema::StandardSchema::Layout {
                        items: vec![
                            (swimos_schema::schema::ItemSchema::ValueItem(#schema), true),
                            #header_schemas
                        ],
                        exhaustive: true
                    }
        ...
        None => {
            if !header_schemas.is_empty() {
                ...
                let schema = StandardSchema::OfKind(quote!(swimos_model::ValueKind::Extant));
                quote!(#schema)
            }
        }
    };
            ...
            quote! {
                {
                    ...
                    let attr_schemas = enumerated.into_iter().fold(Vec::with_capacity(len), |mut vec, variant| {
                        let schema = swimos_schema::schema::text::TextSchema::exact(variant);
                        vec.push(schema);
                        vec
                    });

                    swimos_schema::schema::attr::AttrSchema::new(swimos_schema::schema::text::TextSchema::Or(attr_schemas), #tag_value_schema)
                }
            }
        }
        l => {
    ...
    quote! {
        swimos_schema::schema::StandardSchema::HeadAttribute {
...
pub fn build_attrs(fields: &[ValidatedField]) -> TokenStream2 {
    ...
    if !attrs.is_empty() {
        attrs = quote! {
            swimos_schema::schema::StandardSchema::HasAttributes {
                attributes: vec![
                    #attrs
                ],
                exhaustive: true,
