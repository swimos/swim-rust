...
fn unlink_value_command_to_envelope() {
    let expected = Envelope::unlink("node", "lane").into_outgoing().unwrap();
...
fn unlinked_value_message_from_envelope() {
    let env = Envelope::unlinked("node", "lane").into_incoming().unwrap();
...
fn sync_value_command_to_envelope() {
    let expected = Envelope::sync("node", "lane").into_outgoing().unwrap();
...
fn linked_value_message_from_envelope() {
    let env = Envelope::linked("node", "lane").into_incoming().unwrap();
    let result = value::from_envelope(env);
...
fn synced_value_message_from_envelope() {
    let env = Envelope::synced("node", "lane").into_incoming().unwrap();
    let result = value::from_envelope(env);
