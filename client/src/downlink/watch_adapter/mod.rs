            ...
            match senders.get(&key) {
                ...
                _ => {
                    if senders.len() > *max_active_keys {
                        let first = ages.iter().next().and_then(|(age, key)| {
                            senders.remove(key).map(|sender| (*age, sender))
                        });
                ...
                if let Some(event) = maybe_event {
                    match event {
                        ...
                        Either::Left(BridgeMessage::Special(action, cb)) => {
                            if !producer_handle_special(&mut sink, action, cb, &mut key_streams)
                                .await
                            {
                        ...
                        Either::Right(modification) => {
                            if let Err(RoutingError::RouterDropped) =
                                sink.send_item(modification).await
                            {
                                //Router was dropped.
                                break;
                            }
