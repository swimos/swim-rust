...
pub trait ReconString {
    fn to_recon_string(&self) -> String;
}

impl ReconString for Value {
    fn to_recon_string(&self) -> String {
        match &self {
            Value::Text(s) => format!("Text.from({:?})", s),
            Value::Extant => "".to_string(),
            Value::Int32Value(i) => format!("Num.from({:?}", i),
            Value::Int64Value(i) => format!("Num.from({:?}", i),
            Value::Float64Value(f) => format!("Num.from({:?}", f),
            Value::BooleanValue(b) => format!("Bool.from({:?}", b),
            Value::Record(attrs, items) => {
                let mut res = String::from("Record.of(");
...
impl ReconString for Attr {
    fn to_recon_string(&self) -> String {
        match &self.value {
            Value::Extant => format!("Attr.of({:?})", self.name),
            _ => format!("Attr.of({:?}, {})", self.name, self.value.to_recon_string()),
        }
    }
}

impl ReconString for Item {
    fn to_recon_string(&self) -> String {
        match &self {
            Item::ValueItem(v) => format!("Slot.of({})", v.to_recon_string()),
            Item::Slot(v1, v2) => format!(
                "Slot.of({}, {})",
                v1.to_recon_string(),
                v2.to_recon_string()
            ),
