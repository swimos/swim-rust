...
use swim_common::form::structural::read::parser::{Span, parse_recognize, ParseError};
...
pub fn map_update() {
    ...
    let result = <Upd as Form>::try_from_value(&Value::of_attr(("remove", Value::record(vec![Item::slot("key", "hello")]))));
    assert_eq!(result, Ok(MapUpdate::Remove(Value::text("hello"))))
