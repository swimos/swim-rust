...
type BidirectionalReceiverRequest = Request<mpsc::Receiver<Envelope>>;
            ...
            match SchemeHostPort::try_from(host.clone()) {
                Ok(target) => {
                    if let Some(addr) = state.table_try_resolve(&target) {
                        ...
                        state.defer_dns_lookup(target, Either::Right(request));
                    }
                }
                _ => {
                    request.send_err_debug(
            ...
            match SchemeHostPort::try_from(host.clone()) {
                Ok(target) => {
                    if let Some(addr) = state.table_try_resolve(&target) {
                        ...
                        state.defer_dns_lookup(target, Either::Left(request));
                    }
                }
                _ => {
                    request.send_err_debug(
