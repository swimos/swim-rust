...
#[test]
fn test_transmute_single_tagged_variant() {
    #[derive(Form, Debug, PartialEq, Clone)]
    enum S {
        A {
            a: i32,
...
#[test]
fn test_transmute_multiple_tagged_variants() {
    #[derive(Form, PartialEq, Default, Debug, Clone)]
    struct BTag(String);

    impl ToString for BTag {
    ...
    let s = S::B {
        c: 1,
        d: BTag(String::from("NestedTag")),
    };

    let rec = Value::Record(
        vec![Attr::of("NestedTag")],
        vec![Item::Slot(Value::text("c"), Value::Int32Value(1))],
    );
    assert_eq!(s.as_value(), rec);
